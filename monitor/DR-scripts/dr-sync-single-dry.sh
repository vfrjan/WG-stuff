...
#for server in `cat /ADMIN/WGBA/DR-scripts/servers-new`;do
 ...
 echo ssh $dr_server /local1/disasterrecovery/stop.sh
 for ((j=1;j<=100;j++)); do
  echo "$j"
  export j
 ...
 ssh $server "rsync --delete -avnR -e 'ssh -p 292' --filter='merge /tmp/${server}-merged-files-$j' / root@$dr_server:/"
 done
echo ssh $dr_server /local1/disasterrecovery/configure.sh
echo  ssh $dr_server /local1/disasterrecovery/start.sh
